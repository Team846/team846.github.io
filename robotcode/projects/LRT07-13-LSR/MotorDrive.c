...
char ServoDriveMotor(encoder *e, char speed);
...
{
#ifdef ROBOT_2006
	gMotorSpeed.cimL = -gMotorSpeed.cimL;		// motor polarity backwards on 2006 bot
#endif

	if ((autonomous_mode && gUserOption.AutonomousRawDrive == 2) ||
		(!autonomous_mode && gUserOption.RawDrive == 2))
	{
		mPWMLeftCIM = removeESCDeadband(gMotorSpeed.cimL);
		mPWMRightCIM = removeESCDeadband(gMotorSpeed.cimR);
		if (gLoop.onSecond) printf("RAW Drive\r\n");
	}
	else
	{	
		int DrivePWMLeft = ServoDriveMotor(&gEncoderLeft, gMotorSpeed.cimL);
		int DrivePWMRight = ServoDriveMotor(&gEncoderRight, gMotorSpeed.cimR);
		
		mPWMLeftCIM = removeESCDeadband(DrivePWMLeft);
		mPWMRightCIM = removeESCDeadband(DrivePWMRight);
		if (gLoop.onSecond) printf("SERVO Drive - pot %d\r\n", mServoDriveGainPot);
	}
}
//****************************************************************************
int GetTurnRate(void)
{
	...
	s.right = kTicksAtMaxRPM*1024 / GetDriveWheelPeriod(&gEncoderRight);
	if (gEncoderRight.direction)
		s.right = -s.right;
	
	s.left = kTicksAtMaxRPM*1024 / GetDriveWheelPeriod(&gEncoderLeft);
...
char ServoDriveMotor(encoder *e, char inputSpeed)
{
...
//{
//	PWMs.cimL = limitDriveMotorCurrent(in, gMotorSpeed.cimL, kMaxVrCIM);
//}
//
//void DriveRightMotors(char in)
//{
//	PWMs.cimR = limitDriveMotorCurrent(in, gMotorSpeed.cimR, kMaxVrCIM);
//}
//
