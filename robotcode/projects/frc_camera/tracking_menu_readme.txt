...
The source code in tracking_menu.c/.h contains optional software
that can be used to interactively change, on-the-fly, the
operating parameters of the tracking software. This software
will only work with the "Bells and Whistles" version of the
tracking software.

By default, the tracking menu is invoked by entering the letter
't' into the "To Port ->" window of the IFI loader's terminal
...
1) Install the tracking.c/.h software using the instructions 
found in tracking_readme.txt. This software can be downloaded
from http://kevin.org/frc.

2) You need to add tracking_menu.c and tracking_menu.h to 
your project. Do this by copying the two files to your project
...
tracking_menu.c. Repeat the above procedure for tracking_menu.h 
under "Header Files".

3) A #include statement for the tracking_menu.h header file 
must be included at the beginning of each source file that 
calls the functions in tracking_menu.c. The statement should 
look like this: #include "tracking_menu.h".

4) Code to detect the tracking menu invocation "hotkey" must
...
Here's a description of the functions in tracking_menu.c:

Tracking_Menu()
This function manages the tracking menu functionality, which
is used to enter tracking initialization and operating 
parameters using a terminal attached to your robot controller.


Tracking_Menu_Print()
This function is responsible for sending the tracking menu 
text to the terminal screen.


PWM_Menu_Print()
This function is responsible for sending the PWM parameters
sub-menu text to the terminal screen.


Save_Tracking_Configuration()
This function builds a tracking configuration structure and
saves it to EEPROM, where it can be retrieved by the tracking
software.
