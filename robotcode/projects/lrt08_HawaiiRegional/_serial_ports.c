#define DAVID_G	//changes made by David Giandomenico
...
char gSerialTx1Filled = 0, gSerialTx2Filled = 0;
...
void Write_Serial_Port_One(unsigned char byte)
{
	if (Tx_1_Queue_Full)
	{
		unsigned char priorCharIndex;
		PIE1bits.TX1IE = 0;
		priorCharIndex = (Tx_1_Queue_Write_Index-1)&TX_1_QUEUE_INDEX_MASK;
		Tx_1_Queue[priorCharIndex] = '?';
		Tx_1_Queue[Tx_1_Queue_Write_Index] = '$';
		PIE1bits.TX1IE = 1;
		return;			//Drop Byte!!
	}
#ifdef _SIMULATOR
	if (Tx_1_Queue_Write_Index == TX_1_QUEUE_INDEX_MASK)
		_asm nop _endasm	//a place to set a breakpoint and inspect the buffer
	if (Tx_1_Queue_Write_Index == 0)
		_asm nop _endasm	//a place to set a breakpoint and inspect the buffer
	
	while(Tx_1_Queue_Full)
	{
		_asm nop _endasm	//a place to set a breakpoint to catch buffer full conditions
	}
#else
	while(Tx_1_Queue_Full)
		gSerialTx1Filled = 1;	// if the queue is full, wait here until space is available
#endif
	// put the byte on the circular queue & post increment the write pointer
...
//DAVID_G: This following is incorrect, and inconsistent with KW's volatile declarations for these variables.
	...
	//DAVID_G Disable TX Interrupts; Byte_Count, Read_Index, Queue_Full, Queue_Empty are all volatile
	//That is, they are changed in the TX interrupt handler.
	//Write_Index is OK; It is NOT changed in the TX interrupt handler.
	PIE1bits.TX1IE = 0;
	...
	//DAVID_G: swap order of marking queue not empty and enabling tx interrupt.
	// As soon as Interrupt is enabled, byte is processed.  Otherwise this byte isn't
	//processed until another byte is added to the queue.
	
	// Since we've just added a byte to the queue, it can't possibly be empty.
	Tx_1_Queue_Empty = FALSE;

	//Enable Interrupt will cause the byte to be processed immediately if the TXREG register is ready to
...
#ifdef _SIMULATOR
	if (Tx_1_Queue_Write_Index == TX_1_QUEUE_INDEX_MASK)
		_asm nop _endasm	//a place to set a breakpoint
	if (Tx_1_Queue_Write_Index == 0)
		_asm nop _endasm	//a place to set a breakpoint
...
#ifdef _SIMULATOR
	if (Tx_2_Queue_Write_Index == TX_2_QUEUE_INDEX_MASK)
		_asm nop _endasm	//a place to set a breakpoint and inspect the buffer
	if (Tx_2_Queue_Write_Index == 0)
		_asm nop _endasm	//a place to set a breakpoint and inspect the buffer
...
#else
	while(Tx_2_Queue_Full)
		gSerialTx2Filled = 1;	// if the queue is full, wait here until space is available
#endif
	PIE3bits.TX2IE = 0;

	Tx_2_Queue[Tx_2_Queue_Write_Index] = byte;
	Tx_2_Queue_Write_Index++;	//increment and wrap
...
//			PIE1bits.TX1IE = 0;	//DAVID_G: Next Send_Byte() will re-enable the interrupts. 
		}
...
void Tx_2_Int_Handler(void)
{
	...
	else
	{
		...
		if(Tx_2_Queue_Read_Index == Tx_2_Queue_Write_Index)
		{
			...
			PIE3bits.TX2IE = 0;	//DAVID_G: Next Send_Byte() will re-enable the interrupts. 
		}
...
int _user_putc(unsigned char byte)
{
