...
void GetMotorSpeeds(void)
{
	...
	gMotorSpeed.fpriceL =	motorSpeedFisherPrice( EncoderLeft.velocity, 	gGearBox.left.cmdGearSpeedToMatch);
	gMotorSpeed.cimR =		motorSpeedCIM( EncoderRight.velocity,			gGearBox.right.cmdGearSpeedToMatch);
	gMotorSpeed.fpriceR =	motorSpeedFisherPrice( EncoderRight.velocity,	gGearBox.right.cmdGearSpeedToMatch);
...
void DriveLeftMotors(char in)
{
	...
	gPWM.fpriceL = limitDriveMotorCurrent(in, gMotorSpeed.fpriceL, kMaxVrFisherPrice);
}

void DriveRightMotors(char in)
{
	gPWM.cimR = limitDriveMotorCurrent(in, gMotorSpeed.cimR, kMaxVrCIM);
	gPWM.fpriceR = limitDriveMotorCurrent(in, gMotorSpeed.fpriceR, kMaxVrFisherPrice);
}
...
/*****************************************************************/
int motorSpeedFisherPrice(char velocity, char gear)
{
	overlay int result;
	overlay char signbit=0;
	if (velocity < 0)
...
/*****************************************************************/
int motorSpeedFisherPriceT(unsigned long timestamp, char signDirection, char gear )
{
	overlay int result;
	...
	if (gear==kHighGear)
		result = ((unsigned short long)127 * kToHighGearFisherPrice)/ (unsigned int)timestamp;
	else	//in LowGear
		result = ((unsigned short long)127 * kToLowGearFisherPrice)/ (unsigned int)timestamp;
