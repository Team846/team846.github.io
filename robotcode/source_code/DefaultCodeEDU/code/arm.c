...
void updateArmLocksAbsolute(void)
{
	if (gArm.forearm.locked)
		...
		if (!isWithinRange(mForearmAbs, gArm.forearm.lastOIcmd, 8))	//input moved?
			if (isWithinRange(gArm.forearm.curInput, gArm.forearm.curPosition, mDegrees2Bits(20)))
	...
	if (gArm.shoulder.locked)
		...
		if (!isWithinRange(mShoulderAbs, gArm.shoulder.lastOIcmd, 8)) //input moved?
			if (isWithinRange(gArm.shoulder.curInput, gArm.shoulder.curPosition, mDegrees2Bits(10)))
...
void UserControlArmsAbsolute(void)
{
	...
	gArm.shoulder.curInput = mapForearmInput(mShoulderAbs);

	updateArmLocksAbsolute();

	if (gLoop.onSecond)
	{
		printf("UserControlArmsAbsolute move to %d / %d Forearm/shoulder\n",
			(int)  gArm.forearm.curInput, (int)gArm.shoulder.curInput);
	}
...
#define kThreshold mDegrees2Bits(10)
...
int MoveArm(int distance, char direction)
{
	...
	if (distance > mDegrees2Bits(20))
		pwm = 127;	//full power
	else if (distance > mDegrees2Bits(1))
...
//		setPoint = ((inputOI-a)*(int)x + (b-inputOI)*(int)w) / (int) (b-a);
		setPoint = ((inputOI-a)*((int)x - (int)w)) / (int) (b-a) + w;
	else
		setPoint=w;
...
void armPresets(void)
{
	...
	{
		buttonPressed=1;
		forearmPos = kForeArmLevel + mDegrees2Bits(-10);
		shoulderPos = kShoulderVertical + mDegrees2Bits(-45);
	}
	if (mBtnArmLoadTetra)	//carry
	{
		buttonPressed=1;
		forearmPos = kForeArmLevel + mDegrees2Bits(30);
		shoulderPos = kShoulderVertical + mDegrees2Bits(-45);
	}
	if (mBtnArmCarryTetra)	//low goal
	{
		buttonPressed=1;
//		forearmPos = kForeArmLevel + mDegrees2Bits(60);
//		shoulderPos = kShoulderVertical + mDegrees2Bits(-15);
		forearmPos = kForeArmLevel + mDegrees2Bits(45);
		shoulderPos = kShoulderVertical + mDegrees2Bits(-45);
	}
	if (mBtnArmRaiseTetra)
	{
		buttonPressed=1;
		forearmPos = kForeArmLevel + mDegrees2Bits(80);
		shoulderPos = kShoulderVertical + mDegrees2Bits(0);
	}
	if (buttonPressed)
	{
